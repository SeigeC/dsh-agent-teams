...
import { accentOf, dependencyLabel, memberDotState, memberInitial, memberStateLabel, memberStatusText, taskStatusLabel, taskTone, teamVisibleTo, } from "./activity-ui.js";
import { relatedTaskIds } from "./activity-model.js";
...
function WorkerNode({ member, tasks, focusedRelated, onFocus, onBlur, onNavigate }) {
    ...
    return (_jsxs("div", { className: css.workerNode, "data-worker-node": true, "data-worker-name": member.name, "data-dimmed": focusedRelated !== null && !involved, "data-hot": focusedRelated !== null && involved, children: [_jsxs("button", { type: "button", className: css.workerHead, onClick: () => { if (member.id !== '')
                    onNavigate(member.id); }, title: `${member.name} · ${memberStatusText(member, tasks)}`, children: [_jsx("span", { className: css.workerAvatar, "data-activity": member.activity, children: memberArtUrl(member.name, member.role) !== null ? (_jsx("img", { className: css.workerArt, src: memberArtUrl(member.name, member.role) ?? '', alt: "", "aria-hidden": true })) : (_jsx("span", { className: css.workerInitial, style: { background: accentOf(member.id) }, children: memberInitial(member.name) })) }), _jsxs("span", { className: css.workerInfo, children: [_jsx("span", { className: css.workerName, children: member.name }), _jsx("span", { className: css.workerRole, children: member.role })] }), _jsx("span", { className: css.workerDot, children: _jsx(StateDot, { state: memberDotState(member, tasks) }) }), member.unread > 0 && _jsx("span", { className: css.unreadPill, children: member.unread })] }), _jsx("div", { className: css.workerStatus, "data-activity": member.activity, children: memberStateLabel(member, tasks) }), _jsxs("div", { className: css.workerTasks, children: [owned.length === 0 && _jsx("span", { className: css.taskEmpty, children: "\u6682\u65E0\u4EFB\u52A1" }), owned.map((task) => (_jsxs("button", { type: "button", className: css.taskChip, "data-state": taskTone(task.state, task.status), "data-dimmed": focusedRelated !== null && !focusedRelated.has(task.id), title: `${task.id} ${task.subject}${task.dependencies.length > 0 ? ` · 依赖 ${dependencyLabel(task, tasks)}` : ''}`, onMouseEnter: () => { onFocus(task.id); }, onMouseLeave: onBlur, onFocus: () => { onFocus(task.id); }, onBlur: onBlur, children: [_jsx("span", { className: css.taskChipId, children: task.id }), _jsx("span", { className: css.taskBadge, "data-state": taskTone(task.state, task.status), children: taskStatusLabel(task.status) })] }, task.id)))] })] }));
}
/**
...
export function FlowBoard({ team, onNavigate }) {
    ...
    const related = useMemo(() => (focusedTaskId === null ? null : relatedTaskIds(focusedTaskId, team.tasks)), [focusedTaskId, team.tasks]);
    ...
    return (_jsxs("section", { className: css.board, "data-board": true, "data-team-id": team.teamId, children: [_jsxs("header", { className: css.boardHead, children: [_jsx("span", { className: css.teamName, title: team.name, children: team.name }), _jsxs("span", { className: css.teamStats, children: [_jsxs("span", { "data-stat": "members", children: [team.members.length, " \u6210\u5458"] }), _jsxs("span", { "data-stat": "tasks", children: [completedCount, "/", team.tasks.length, " \u5B8C\u6210"] }), _jsxs("span", { "data-stat": "messages", children: [team.messageCount, " \u6D88\u606F"] })] })] }), unassigned.length > 0 && (_jsxs("div", { className: css.unassignedBar, children: [_jsx("span", { className: css.unassignedLabel, children: "\u5F85\u8BA4\u9886" }), unassigned.map((task) => (_jsxs("button", { type: "button", className: css.taskChip, "data-state": taskTone(task.state, task.status), "data-dimmed": related !== null && !related.has(task.id), title: `${task.id} ${task.subject}`, onMouseEnter: () => { setFocusedTaskId(task.id); }, onMouseLeave: () => { setFocusedTaskId(null); }, onFocus: () => { setFocusedTaskId(task.id); }, onBlur: () => { setFocusedTaskId(null); }, children: [_jsx("span", { className: css.taskChipId, children: task.id }), _jsx("span", { className: css.taskBadge, "data-state": taskTone(task.state, task.status), children: taskStatusLabel(task.status) })] }, task.id)))] })), _jsxs("div", { className: css.flowArea, ref: containerRef, children: [_jsxs("svg", { className: css.flowSvg, width: containerSize.width, height: containerSize.height, "aria-hidden": true, children: [_jsx("defs", { children: _jsx("marker", { id: `at-flow-arrow-${team.teamId}`, viewBox: "0 0 10 10", refX: "9", refY: "5", markerWidth: "7", markerHeight: "7", orient: "auto-start-reverse", children: _jsx("path", { d: "M 0 0 L 10 5 L 0 10 z", className: css.edgeArrow }) }) }), edges.map((edge) => {
                                ...
                                return (_jsxs("g", { children: [_jsx("path", { className: css.edgePath, "data-dimmed": dimmed, "data-hot": hot, d: path, markerEnd: `url(#at-flow-arrow-${team.teamId})` }), _jsxs("g", { className: css.edgeLabel, "data-dimmed": dimmed, "data-hot": hot, transform: `translate(${(x1 + x2) / 2} ${midY})`, onMouseEnter: () => { setFocusedTaskId(edge.task.id); }, onMouseLeave: () => { setFocusedTaskId(null); }, onFocus: () => { setFocusedTaskId(edge.task.id); }, onBlur: () => { setFocusedTaskId(null); }, role: "button", tabIndex: 0, "aria-label": `需求 ${edge.task.id} 从 ${edge.from} 流转到 ${edge.to}`, children: [_jsx("rect", { className: css.edgeLabelBg, width: labelWidth, height: 20, rx: 10 }), _jsx("text", { className: css.edgeLabelText, x: labelWidth / 2, y: 14, textAnchor: "middle", children: edge.task.id }), _jsx("title", { children: `${edge.task.id} ${edge.task.subject}：${edge.from} → ${edge.to}` })] })] }, edge.id));
                            })] }), _jsx("div", { className: css.workerGrid, children: team.members.map((member) => (_jsx(WorkerNode, { member: member, tasks: team.tasks, focusedRelated: related, onFocus: setFocusedTaskId, onBlur: () => { setFocusedTaskId(null); }, onNavigate: onNavigate }, member.id))) })] })] }));
}
