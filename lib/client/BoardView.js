...
function FlowEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, data }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(BaseEdge, { id: id, path: path, markerEnd: hot ? 'url(#dsh-agent-teams-arrow-hot)' : 'url(#dsh-agent-teams-arrow-base)', style: {
                ...
                } }), _jsx(EdgeLabelRenderer, { children: _jsx("div", { className: css.edgeLabel, "data-hot": hot, "data-dimmed": hidden ? false : dimmed, style: {
                        position: 'absolute',
    ...
    useEffect(() => {
        ...
        const measure = () => {
            ...
            const orbEl = holderTask !== undefined
                ? [...orb.querySelectorAll('button')].find((button) => button.textContent.trim() === holderTask.id)
                : undefined;
