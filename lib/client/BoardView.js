...
function RequirementRail({ team, focusedTaskId, related, onFocus, onBlur }) {
    return (_jsx("aside", { className: css.requirementRail, "aria-label": "\u9700\u6C42\u5217\u8868", children: RAIL_GROUPS.map((group) => {
            const tasks = team.tasks.filter(group.match);
            if (tasks.length === 0)
                return null;
            return (_jsxs("div", { className: css.railGroup, children: [_jsxs("header", { className: css.railGroupHead, children: [_jsx("span", { children: group.label }), _jsx("span", { className: css.railCount, children: tasks.length })] }), tasks.map((task) => {
                        const tone = taskTone(task.state, task.status);
                        const hot = focusedTaskId !== null && related?.has(task.id) === true;
                        const dimmed = focusedTaskId !== null && !hot;
                        return (_jsxs("button", { type: "button", className: css.railItem, "data-state": tone, "data-hot": hot, "data-dimmed": dimmed, title: `${task.id} ${task.subject}${task.dependencies.length > 0 ? ` · 依赖 ${dependencyLabel(task, team.tasks)}` : ''}`, onMouseEnter: () => { onFocus(task.id); }, onMouseLeave: onBlur, onFocus: () => { onFocus(task.id); }, onBlur: onBlur, children: [_jsx("span", { className: css.railItemId, children: task.id }), _jsx("span", { className: css.railItemSubject, children: task.subject }), _jsx("span", { className: css.railItemOwner, children: task.assignee !== '' ? `→ ${task.assignee}` : '待认领' })] }, task.id));
                    })] }, group.key));
        }) }));
}
/**
...
export function FlowBoard({ team, onNavigate }) {
    ...
    return (_jsxs("section", { className: css.board, "data-board": true, "data-team-id": team.teamId, children: [_jsxs("header", { className: css.boardHead, children: [_jsx("span", { className: css.teamName, title: team.name, children: team.name }), _jsxs("span", { className: css.teamStats, children: [_jsxs("span", { "data-stat": "members", children: [team.members.length, " \u6210\u5458"] }), _jsxs("span", { "data-stat": "tasks", children: [completedCount, "/", team.tasks.length, " \u5B8C\u6210"] }), _jsxs("span", { "data-stat": "messages", children: [team.messageCount, " \u6D88\u606F"] })] })] }), _jsxs("div", { className: css.boardLayout, children: [_jsx(RequirementRail, { team: team, focusedTaskId: focusedTaskId, related: related, onFocus: setFocusedTaskId, onBlur: () => { setFocusedTaskId(null); } }), _jsxs("div", { className: css.flowArea, ref: containerRef, children: [_jsxs("svg", { className: css.flowSvg, width: containerSize.width, height: containerSize.height, "aria-hidden": true, children: [_jsx("defs", { children: _jsx("marker", { id: `at-flow-arrow-${team.teamId}`, viewBox: "0 0 10 10", refX: "9", refY: "5", markerWidth: "7", markerHeight: "7", orient: "auto-start-reverse", children: _jsx("path", { d: "M 0 0 L 10 5 L 0 10 z", className: css.edgeArrow }) }) }), edges.map((edge) => {
