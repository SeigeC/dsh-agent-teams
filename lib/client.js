						...
						(0, react_jsx_runtime.jsxs)("svg", {
							...
							children: [
								...
								edges.map((edge) => {
									...
									const path = routeConnection(x1, y1, x2, y2, [...nodeRects.entries()].filter(([name]) => name !== edge.from && name !== edge.to).map(([, rect]) => ({
								...
								requirements.map((requirement) => {
									...
									const path = routeConnection(x1, y1, x2, y2, [...nodeRects.entries()].filter(([name]) => name !== holder).map(([, rect]) => ({
										left: rect.left - containerRect.left,
										top: rect.top - containerRect.top,
										right: rect.left + rect.width - containerRect.left,
										bottom: rect.top + rect.height - containerRect.top
