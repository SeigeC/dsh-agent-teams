...
/** Requirement rail groups in display order (everything the team carries). */
		const RAIL_GROUPS = [
			{
				key: "pending",
				label: "未开始",
				match: (task) => task.status === "pending" || task.status === "claimed"
			},
			{
				key: "running",
				label: "进行中",
			...
			},
			{
				key: "failed",
				label: "异常",
				match: (task) => task.status === "failed" || task.status === "cancelled"
			}
		];
		/**
		...
		*/
		function RequirementRail({ team, focusedTaskId, related, onFocus, onBlur }) {
			return (0, react_jsx_runtime.jsx)("aside", {
				...
				"aria-label": "需求列表",
				children: RAIL_GROUPS.map((group) => {
							...
							return (0, react_jsx_runtime.jsxs)("button", {
								...
								children: [
									...
									(0, react_jsx_runtime.jsx)("span", {
										...
										children: task.assignee !== "" ? `→ ${task.assignee}` : "待认领"
									})
								]
							}, task.id);
						})]
		...
		*/
		function FlowBoard({ team, onNavigate }) {
			...
			const [railRects, setRailRects] = (0, react.useState)(/* @__PURE__ */ new Map());
			...
			(0, react.useLayoutEffect)(() => {
				const update = () => {
					...
					const railMap = /* @__PURE__ */ new Map();
					for (const el of container.querySelectorAll("[data-rail-task]")) {
						const taskId = el.dataset.railTask;
						if (taskId !== void 0) railMap.set(taskId, el.getBoundingClientRect());
					}
					setNodeRects(map);
						...
						(0, react_jsx_runtime.jsxs)("svg", {
							...
							children: [
								...
								team.tasks.map((task) => {
									...
									if (rail === void 0 || orb === void 0 || containerRect === null) return null;
									...
									const bend = Math.max(16, Math.abs(x1 - x2) * .4);
									const path = `M ${x1} ${y1} C ${x1 + bend} ${y1}, ${x2 - bend} ${y2}, ${x2} ${y2}`;
									const hot = related !== null && related.has(task.id);
									const dimmed = related !== null && !hot;
									return (0, react_jsx_runtime.jsx)("path", {
										className: BoardView_module_css_default.handlerLink,
										"data-hot": hot,
										"data-dimmed": dimmed,
										d: path
									}, `handler:${task.id}`);
