...
`;
		...
		* stage orbs (with the flow edges between them). */
		function WorkerNode({ member, tasks, focusedRelated, onFocus, onBlur, onNavigate }) {
			...
			const visible = (focusedRelated !== null ? owned.filter((task) => focusedRelated.has(task.id)) : owned.filter((task) => task.status === "in_progress")).slice(0, TASK_ORB_COUNT);
		...
		* requirement. Requirements are independent — no cross-requirement links. */
		function buildRequirements(tasks) {
			...
			return [...byName.entries()].map(([key, stages]) => ({
				id: key,
		...
		* Flow's controlled props (see WorkerOrbData). */
		const HoverFocusContext = (0, react.createContext)({
			id: null,
			related: null
		});
		...
		* mini node (the requirement's stage balls), not orb edge to orb edge. */
		function WorkerOrbNode({ data }) {
			const { related } = (0, react.useContext)(HoverFocusContext);
			const all = data.tasks.filter((task) => task.assignee === data.member.name).slice(0, TASK_ORB_COUNT);
			const angleStep = all.length <= 1 ? 0 : 360 / all.length;
			return (0, react_jsx_runtime.jsxs)("div", {
				className: BoardView_module_css_default.nodeWrap,
				children: [all.map((task, index) => {
					...
					return (0, react_jsx_runtime.jsxs)(react.Fragment, { children: [(0, react_jsx_runtime.jsx)(Handle, {
						id: `src-${task.id}`,
						type: "source",
		...
		* by hover state. */
		function FlowEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, data }) {
			...
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)(BaseEdge, {
				id,
				path,
			...
			}), (0, react_jsx_runtime.jsx)(EdgeLabelRenderer, { children: (0, react_jsx_runtime.jsx)("div", {
		...
		*/
		function FlowBoard({ team, onNavigate }) {
			...
			* item to its handler orb, routed around every other orb. */
			...
			const focusRequirementOf = (0, react.useCallback)((taskId) => {
			...
			(0, react.useEffect)(() => {
				...
				const measure = () => {
					...
					const orb = origin !== void 0 && origin.assignee !== "" ? layout.querySelector(`[data-worker-node][data-worker-name="${origin.assignee}"]`) : null;
					if (railItem === null || orb === null || origin === void 0) {
						setHoverLinkPath(null);
						return;
					}
					...
					const orbEl = [...orb.querySelectorAll("button")].find((button) => button.textContent.trim() === origin.id);
					const targetRect = orbEl !== void 0 ? orbEl.getBoundingClientRect() : void 0;
					const x2 = targetRect !== void 0 ? targetRect.left - layoutRect.left : orbRect.left - layoutRect.left;
					const y2 = targetRect !== void 0 ? targetRect.top + targetRect.height / 2 - layoutRect.top : orbRect.top + orbRect.height / 2 - layoutRect.top;
					const obstacles = [...layout.querySelectorAll("[data-worker-node]")].filter((el) => el.dataset.workerName !== origin.assignee).map((el) => {
				...
				children: [(0, react_jsx_runtime.jsxs)("header", {
					...
					children: [
						...
						(0, react_jsx_runtime.jsxs)("div", {
							...
							children: [(0, react_jsx_runtime.jsx)(EdgeMarkerDefs, {}), (0, react_jsx_runtime.jsx)(HoverFocusContext.Provider, {
								value: {
									id: focusedRequirementId,
									related
								},
		...
		* hover events. */
		const BOARD_OPEN_ATTRIBUTE = "data-agent-teams-board-open";
			...
			* React Flow node payloads (and their DOM) on every board re-render. */
			...
			(0, react.useEffect)(() => {
				...
				const tick = async () => {
					...
					inFlight = true;
					try {
						const response = await fetch(STATE_URL$1, { cache: "no-store" });
						if (response.ok) {
							const body = await response.json();
							if (!cancelled && Array.isArray(body.teams)) setTeams((previous) => {
								const next = body.teams;
								if (previous.length === next.length && JSON.stringify(previous) === JSON.stringify(next)) return previous;
								return next;
							});
						}
					} catch {} finally {
		...
		function useBoardOpen() {
			return (0, react.useSyncExternalStore)((onChange) => {
				...
				observer.observe(document.documentElement, {
					attributes: true,
					attributeFilter: [BOARD_OPEN_ATTRIBUTE]
				});
				return () => {
					observer.disconnect();
				};
